...
одним относительным смещением на ячейку. Результат я назвал RelF, от
Relative Forth. (Ячейка — это машинное слово, на котором построен Forth:
четыре байта на 32-разрядной системе, восемь на 64-разрядной.) Смысл был
в скорости: пройти по одному смещению — меньше работы, чем распаковать
...
Дальше — что я пробовал, в том порядке, в каком пробовал, включая две
попытки, которые не сработали, и почему.
...
вся проблема именно в этой асимметрии. Таблиц в итоге две, направленных
в разные стороны:

    у движка        номер -> адрес    строится при загрузке, в памяти C
    у компилятора   адрес -> номер    строит образ, в куче
