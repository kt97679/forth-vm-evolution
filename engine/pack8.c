/*
 *  pack8.c - RelF's cell engine with the TAGGED-BYTE pack added.
 *
 *  This is the scheme ENCODING-COMPARISON.md calls "tagged byte
 *  (8-bit x3/x7)" and PROGRESS.md's Iteration 157 rejected on dispatch
 *  cost, having measured it at 1.60x on x86-64 and 1.90x on i386 in a
 *  synthetic loop. It was never built. This file builds it.
 *
 *  ENCODING. A cell is one of three things, decided by its low bits:
 *
 *    xxxx xx00   a CALL. The cell IS the relative offset, as in RelF.
 *    nnnn nn01   a PRIMITIVE, value 1 + index*CELL_BYTES, as in RelF.
 *    cccc cc11   a PACK: a tag byte, then one opcode byte per cell byte.
 *
 *  In the tag byte, bit 0 and bit 1 are the marker, bits 2..4 hold the
 *  count less one, so a pack carries 1..7 opcodes on an 8-byte cell and
 *  1..3 on a 4-byte one. A plain primitive cell always has bit 1 clear,
 *  because its value is 1 + index*CELL_BYTES and CELL_BYTES is 4 or 8,
 *  so the two forms cannot be confused and the cell encoding survives
 *  underneath the packed one.
 *
 *  WHY THE CELL FORM SURVIVES. Because it means a PACK8 image is a
 *  superset of a RelF image, so the existing compiler already emits
 *  valid - if unpacked - PACK8 code. That is what lets this stage
 *  self-host without a compiler overlay.
 *
 *  WHAT THE PACK COSTS, which is the whole question. Every dispatch now
 *  begins with a test of the pack counter, and a cell fetch that finds a
 *  pack takes a second test to tell it from a plain primitive. That is
 *  precisely Iteration 134's objection - "a second data-dependent test
 *  on the two hottest paths" - and it is here in the source rather than
 *  in a model.
 */
/*
 *  RelF - Relative Forth
...
/*  The shift register. `pk` holds the opcode bytes of the pack still to
 *  run and `pkn` how many are left, so a dispatch inside a pack costs a
 *  shift and a mask instead of a memory fetch - which is the density
 *  argument - but EVERY dispatch now pays the test that asks whether we
 *  are in a pack at all, which is the cost.
 *
 *  A pack physically occupies as many cells as it has opcodes, and `ip`
 *  steps over all of them. The cells after the first are dead: they are
 *  never fetched and never executed. That is deliberate - see
 *  tools/pack8.py for why this build measures dispatch honestly while
 *  leaving the image the same size.  */
#define NEXT() do { \
        if (pkn) { pkn--; t = pk & 0xFF; pk >>= 8; goto *dispatch[t]; } \
        t = CELL(ip); \
        if (t & 1) { \
            if (t & 2) { \
                pkn = (int)((t >> 2) & 7) + 1; \
                ip += (UNS64)CELL_BYTES * (UNS64)pkn; \
                pk = t >> 8; pkn--; t = pk & 0xFF; pk >>= 8; \
                goto *dispatch[t]; \
            } \
            ip += CELL_BYTES; \
    ...
    UNS64 pk = 0;       /* opcode bytes of the pack still to run */
