/*
 *  pack4.c - RelF's cell engine with the TAGGED-NIBBLE pack added.
 *
 *  ENCODING-COMPARISON.md calls this "tagged nibble (4-bit x7)" and
 *  Iteration 157 rejected it at 1.65x dispatch on x86-64 and 2.05x on
 *  i386, from a synthetic loop, without building it. This builds it.
 *
 *  It is the sibling of pack8.c and differs in one thing: an opcode is
 *  four bits instead of eight, so twice as many fit in a cell and the
 *  alphabet is limited to SIXTEEN primitives. Everything outside that
 *  alphabet stays a plain cell, which is also what ends a run.
 *
 *  ENCODING. A cell is one of three things, by its low bits:
 *
 *    xxxx xx00   a CALL. The cell IS the relative offset, as in RelF.
 *    nnnn nn01   a PRIMITIVE, value 1 + index*CELL_BYTES, as in RelF.
 *    cccc cc11   a PACK: a tag byte, then two opcode nibbles per
 *                remaining byte, low nibble first.
 *
 *  The tag is a BYTE, not the nibble the original proposal describes,
 *  and that is forced rather than chosen. The low two bits have to
 *  separate a pack from a plain primitive cell, whose value is
 *  1 + index*CELL_BYTES and so always has bit 1 clear. A four-bit tag
 *  would spend two of its bits on that marker and have two left for a
 *  length, which caps a pack at four operations and throws away most of
 *  what the narrow opcode buys. With a byte tag, bits 2..5 hold the
 *  count less one: up to 6 operations in a 4-byte cell and 14 in an
 *  8-byte one, against pack8.c's 3 and 7.
 *
 *  THE ALPHABET is not fixed here. tools/pack4.py counts the primitives
 *  actually used in the image being packed, takes the sixteen most
 *  frequent that can be packed at all, and writes pack4-alphabet.h.
 *  Choosing it by hand from the frequency table in ENCODING-COMPARISON
 *  would have meant quoting a census taken over a different word set.
 */
/*
 *  RelF - Relative Forth
...
static void virtual_machine(void) {
    ...
    /*  Nibble value -> primitive index, generated from the image that
     *  is being run. See tools/pack4.py.  */
    static const unsigned char alpha[16] = {
#include "pack4-alphabet.h"
    };

/*  `pk` holds the opcode nibbles still to run and `pkn` how many. A
 *  dispatch inside a pack is a shift, a mask and one table lookup more
 *  than pack8.c needs - the extra lookup is the price of the narrow
 *  field, and it is on the hot path.
 *
 *  A pack occupies as many cells as it has operations and `ip` steps
 *  over all of them; the cells after the first are dead and never
 *  fetched. tools/pack4.py explains why they are left in place.  */
#define NEXT() do { \
        if (pkn) { pkn--; t = alpha[pk & 0xF]; pk >>= 4; goto *dispatch[t]; } \
        t = CELL(ip); \
        if (t & 1) { \
            if (t & 2) { \
                pkn = (int)((t >> 2) & 15) + 1; \
                ip += (UNS64)CELL_BYTES * (UNS64)pkn; \
                pk = t >> 8; pkn--; \
                t = alpha[pk & 0xF]; pk >>= 4; \
                goto *dispatch[t]; \
            } \
    ...
    UNS64 pk = 0;       /* opcode nibbles of the pack still to run */
    int pkn = 0;        /* how many of them are left */

next:
    NEXT();

L_noop:    /* noop    */ NEXT();
L_exit:    /* exit    */ pkn = 0; ip = RS; rp += CELL_BYTES; NEXT();
